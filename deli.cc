	...
	while(1){
		...
		if(killThread == 1){
			...
			if(numCashiers<maxOrdersOnBoard){
				//fprintf(stderr, "submitOrder: if: if: enter if statement. numCashiers (%d) is < maxOrdersOnBoard (%d) \n", numCashiers, maxOrdersOnBoard);
	 			maxOrdersOnBoard = numCashiers;
	 			thread_signal(deliLock, makerCV);
	 		}else{
	 			//fprintf(stderr, "submitOrder: if: else: signalling other cashiers. numCashiersis %d and maxOrdersOnBoard is %d\n", numCashiers, maxOrdersOnBoard);
	 			for(unsigned int i=0;i<initialNumCashiers;i++){
	 				if(arr[i]==true){
	 					//fprintf(stderr, "im signalling cashier %d\n", i);
